...
**CI 알림 배선 (2026-07-31)**: 주간 워크플로가 커밋 *전에* `_amend_report.py` 를 돌려 **`법령MST` 가 실제로 바뀐 파일만** 골라 조문 단위로 푼다. 개정이 있으면 ① 커밋 제목이 무엇이 바뀌었는지 말하고(`chore: 원자력안전법 시행규칙 제121조(건강진단) 개정`) ② **GitHub 이슈**를 연다(`[법령개정] …`) — GitHub 알림이 메일·앱으로 도달한다. 별도 secret 불필요(`GITHUB_TOKEN`).
