...
> ⚠️ **운영 배포는 아직 미해결.** 이 측정은 로컬 Ollama 기준이고, Vercel/Supabase 서빙에서는 질의 임베딩을 같은 모델로 만들 수단이 필요하다(임베딩 API 또는 자체 호스팅 추론). Voyage/OpenAI 로 갈지, qwen3 를 호스팅할지는 미결 — 이 표가 그 비교의 기준선이다.
