...
CLI:
  python3 _watchlist.py --fetch-list   # name<TAB>type  (update_laws.sh 소비)
  python3 _watchlist.py --ids          # law_id  name  type  family  (확인용)
"""
...
def members():
    """[{family, name, type, law_id}] 평탄 리스트."""
...
def main(argv):
    ...
    if '--fetch-list' in argv:
        ...
        for m in ms:
            print(f"{m['law_id']}\t{m['name']}\t{m['type']}\t{m['family']}")
    else:
        print(__doc__)
        print(f"멤버 {len(ms)}건 / 법률패밀리 "
